...
pub fn start(todo_path: PathBuf, token: String, port: u16) -> Result<ShareInfo> {
    ...
    thread::Builder::new()
        .name("tuxtime-capture-server".into())
        .spawn(move || {
            for request in server.incoming_requests() {
                if let Err(e) = handle(request, &path_arc, &token_arc) {
                    eprintln!("tuxtime capture: request error: {e}");
                }
            }
        })
...
mod tests {
    ...
    fn tasks_view_separates_open_and_inbox() {
        let dir =
            std::env::temp_dir().join(format!("tuxtime-serve-tasks-view-{}", std::process::id()));
    ...
    fn start_binds_and_serves() {
        let dir = std::env::temp_dir().join(format!("tuxtime-serve-start-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let todo_path = dir.join("todo.txt");
