...
pub(crate) fn mode_hint(app: &App) -> String {
    ...
    let default_hint: &'static str = if matches!(app.nav.view, View::Timesheet) {
        ...
        "j/k · n new · Enter timer · t timer · T interrupt · x done · / search · ? help · u undo · q quit"
    };
    ...
    match app.nav.mode {
        Mode::Screen(screen) => match screen {
            ...
            Screen::Settings => {
                "Esc/ ,/ q dismiss  ·  i idle nudge  ·  l long timer nudge  ·  I duration badge  ·  + new-task prefill  ·  e edit cursor  ·  t enter timer  ·  x done-no-time prompt  ·  O log date".to_string()
            }
