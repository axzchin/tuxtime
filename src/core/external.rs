...
impl Store {
    ...
    pub fn drain_inbox(&mut self) -> DrainReport {
        ...
        // Coordinate with `tuxtime serve`'s POST handler (and other tuxtime
...
mod tests {
    ...
    fn build_store_with_dir(todo_raw: &str) -> (Store, std::path::PathBuf, std::path::PathBuf) {
        ...
        let dir = std::env::temp_dir().join(format!("tuxtime-inbox-{}-{}", std::process::id(), n));
    ...
    fn drain_merges_natural_language_lines() {
        ...
        assert!(!dir.join("inbox.txt.tuxtime-staging").exists());
    }
    ...
    fn drain_recovers_existing_staging_file() {
        let (mut store, dir, _) = build_store_with_dir("a\n");
        std::fs::write(dir.join("inbox.txt.tuxtime-staging"), "recovered task\n").unwrap();
