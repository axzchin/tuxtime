...
//! folder, `tuxtime serve`'s POST handler) drop one task per line into a
...
pub const STAGING_FILENAME: &str = "inbox.txt.tuxtime-staging";
pub const LOCK_FILENAME: &str = "inbox.txt.tuxtime-lock";
...
/// `inbox.txt` → `inbox.txt.tuxtime-staging` *before* reading, so any
...
/// `tuxtime serve` POST handler (around its append) and the TUI drain
...
mod tests {
    ...
    fn staging_path_for_uses_distinct_name() {
        ...
        assert_eq!(
            staging_path_for(&p),
            PathBuf::from("/tmp/work/inbox.txt.tuxtime-staging"),
        );
    }
    ...
    fn acquire_lock_blocks_a_concurrent_holder() {
        ...
        let dir = std::env::temp_dir().join(format!("tuxtime-inbox-lock-{}", std::process::id()));
